...
class Client:
    ...
    def setupMovie(self):
        if self.state == self.INIT:
            self.chooseTransportProtocol()

        elif self.state == self.PLAYING:
            if tkMessageBox.askokcancel(
                "Reset Video?",
                "Do you want to stop current playback and setup from the beginning?",
            ):
            ...
            if tkMessageBox.askokcancel(
                "Reset Video?",
                "Do you want to clear buffer and setup from the beginning?",
            ):
    ...
    def chooseTransportProtocol(self):
        ...
        btn_ok = Button(
            ...
            text="OK",
            width=12,
            command=self.confirmTransportProtocol,
            fg="#FFFFFF",
            bg="#007ACC",
            activeforeground="#FFFFFF",
    ...
    def renderClientBufferLoop(self):
        ...
        with self.bufferLock:
            if self.isBuffering and not self.isDraining:
                if len(self.frameBuffer) >= self.minBufferSize:
                    ...
                    self.master.after(
                        40, self.renderClientBufferLoop
                    )  # Check again after 40ms
                    return
        ...
        self.master.after(
            40, self.renderClientBufferLoop
        )  # Schedule next frame render after 40ms (25fps)
                            ...
                            with self.bufferLock:
                                ...
                                if len(self.frameBuffer) > 100:
                                    self.frameBuffer.pop(
                                        0
                                    )  # Discard oldest frame if buffer exceeds size
